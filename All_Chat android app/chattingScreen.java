...
public class chattingScreen extends AppCompatActivity {
    ...
    public class checkLastMessage extends AsyncTask<Void,Void,Void>{
        ...
        protected Void doInBackground(Void... voids) {
            ...
            String link="http://192.168.1.17/All_Chat/Mobile/checkLastMessage.php?check=fromMobile1090&username="+username+"&password="+password+"&tableName="+tableName+"&lastIndex="+lastIndex;
    ...
    public class getMessagesConn extends AsyncTask<Void,Void,Void>{
        ...
        protected Void doInBackground(Void... voids) {
            ...
            String link="http://192.168.1.17/All_Chat/Mobile/getMessages.php?check=fromMobile1090&username="+username+"&password="+password+"&tableName="+tableName;
        ...
        protected void onPostExecute(Void aVoid) {
            if(isOk){
                String msg2=msg.substring(0,msg.length()-1);
                String arr1[]=msg2.split("&");


                if(arr1.length>0){
                    for(int i=0;i<arr1.length;i++){
                        String arr2[]=arr1[i].split("\\|");
                        setMessage(arr2[0],arr2[1]);
                        lastIndex++;
                    }
    ...
    public class conn extends AsyncTask<Void,Void,Void> {
        ...
        @Override
        protected Void doInBackground(Void... voids) {
            String link="http://192.168.1.17/All_Chat/Mobile/getFriendData.php?check=fromMobile1090&username="+username+"&password="+password+"&friendUsername="+friendUsername;
            try{
                URL url = new URL(link);
                HttpURLConnection con = (HttpURLConnection) url.openConnection();
                con.setRequestMethod("GET");
    ...
    private void refreshFriendData(){
        ...
        Picasso.get().load("http://192.168.1.17/All_Chat/Extra/styles/images/"+friendPicName+".png").into(friendPicField);
    }
    ...
    public class sendConn extends AsyncTask<Void,Void,Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            String link="http://192.168.1.17/All_Chat/Mobile/sendMessage.php?check=fromMobile1090&username="+username+"&password="+password+"&tableName="+tableName+"&message="+message+"&lastIndex="+lastIndex;
