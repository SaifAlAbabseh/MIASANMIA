...
public class mainScreen extends AppCompatActivity {
    ...
    public void goToProfileEditScreen(View v){
        Intent browse = new Intent( Intent.ACTION_VIEW , Uri.parse( "http://192.168.1.17/All_Chat/uploadPic.php?username="+username+"&password="+password+"&temp=javaToWeb1090" ) );
    ...
    public class addFriendConn extends AsyncTask<Void,Void,Void>{
        ...
        protected Void doInBackground(Void... voids) {
            String link="http://192.168.1.17/All_Chat/Mobile/addFriend.php?check=fromMobile1090&username="+username+"&password="+password+"&friendUsername="+friendUsername;
            try{
    ...
    public class updateAva extends AsyncTask<Void,Void,Void>{
        ...
        @Override
        protected Void doInBackground(Void... voids) {
            String link="http://192.168.1.17/All_Chat/Mobile/updateAvailability.php?check=fromMobile1090&username="+username+"&password="+password+"&which=1";
            try{
                URL url = new URL(link);
                HttpURLConnection con = (HttpURLConnection) url.openConnection();
                con.setRequestMethod("GET");
    ...
    public void mainpopupmenu(View v){
        ...
        Picasso.get().load("http://192.168.1.17/All_Chat/Extra/styles/images/"+profilePicName+".png").into(profilePic);
        d.show();
    }
    public class conn extends AsyncTask<Void,Void,Void> {
        ...
        @Override
        protected Void doInBackground(Void... voids) {
            String link="http://192.168.1.17/All_Chat/Mobile/getProfilePic.php?check=fromMobile1090&username="+username+"&password="+password;
            String link2="http://192.168.1.17/All_Chat/Mobile/getFriends.php?check=fromMobile1090&username="+username+"&password="+password;
            try{
                URL url = new URL(link);
                HttpURLConnection con = (HttpURLConnection) url.openConnection();
                con.setRequestMethod("GET");
        ...
        private void setFriend(String username,String picture,String whichSide){
            ...
            Picasso.get().load("http://192.168.1.17/All_Chat/Extra/styles/images/"+picture+".png").into(pic);
